...
#load corpus
corpus = io.open(r'test.conllu', "r", encoding="utf-8")
#load stanza
...
#create output file
filename = time.strftime("results/classify-bert-deprel-filter-%Y%m%d-%H%M%S-",time.localtime())+'.txt'
...
for sen in conllu.parse_incr(corpus):
  ...
  mut_sens_ids = sen_conllu.mutation_bert_insert_word_persent()
  mut_num += len(mut_sens_ids)
  for mut_sen_id in mut_sens_ids:
    temp_res = mut_pos_compare_appendid_deprel_persent_res(sen_stanza, PTF_Sen(nlp(mut_sen_id[0]).sentences[0].words), mut_sen_id[1])
    if len(temp_res) > 0:
      for error_i in temp_res:
